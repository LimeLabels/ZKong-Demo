...
class NCRAPIClient:
    ...
    def _generate_signature(
        ...
        signature = hmac.new(
            signing_key.encode("utf-8"), signable_content.encode("utf-8"), hashlib.sha512
        ).digest()
    ...
    async def create_product(
        ...
        if barcode and barcode != item_code:
            item_data.packageIdentifiers = [{"type": "UPC", "value": barcode}]
    ...
    async def list_items(
        ...
        page_number: int = 0,
        page_size: int = 200,
        item_code_pattern: str | None = None,
    ) -> dict[str, Any]:
        """
    ...
    async def delete_product(
        ...
        item_data = ItemWriteData(
            ...
            shortDescription=MultiLanguageTextData.from_single_text(
                "Product deleted"
            ),  # Required field (use placeholder instead of empty)
