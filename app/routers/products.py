...
async def search_products(
    ...
    try:
        ...
        if q:
            ...
            db_results = supabase_service.client.table("products").select("*").or_(
                f"barcode.ilike.%{q}%,sku.ilike.%{q}%,title.ilike.%{q}%"
            ).limit(limit).execute()
        ...
        try:
                ...
                for product in shopify_data.get("products", [])[:limit]:
                    ...
                    for variant in product.get("variants", []):
                        ...
                        if q:
                            ...
                            barcode_match = q.lower() in (variant.get("barcode") or "").lower()
            ...
            logger.warning("Failed to search Shopify API, using database results only", error=str(e))
